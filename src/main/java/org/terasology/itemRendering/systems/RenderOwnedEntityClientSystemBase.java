...
public abstract class RenderOwnedEntityClientSystemBase extends BaseComponentSystem {
    ...
    protected void addRenderingComponents(EntityRef owningEntity, EntityRef item,
                                          RenderOwnedEntityDetails renderOwnedEntityDetails) {
        if (item.exists()) {
            // this is inherently evil,  but multiplayer acts strangely
            item.setOwner(owningEntity);

            RenderItemComponent renderItemTransform = createRenderItemComponent(owningEntity, item,
                    renderOwnedEntityDetails);
    ...
    public RenderItemComponent createRenderItemComponent(EntityRef owningEntity, EntityRef item,
                                                         RenderOwnedEntityDetails renderOwnedEntityDetails) {
